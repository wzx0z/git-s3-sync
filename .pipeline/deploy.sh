...
if [ $sha_before ];then
  ...
  git diff-tree --no-commit-id --name-status -r $sha_before $sha | grep -vE '\.pipeline/.*' | while read status filename; do
  ...
  aws s3 sync . "s3://$s3bucket" --exclude ".git/*" --exclude ".pipeline/*" $cliparams
fi
